...
const Dashboard = () => {
  ...
  const stats = useMemo(() => {
    ...
    return { myTasks, myCompletedTasks, myInProgressTasks, myPendingTasks, overdueTasks };
  }, [accessibleTasks, currentUserId]);

  const quickStats = [
    { title: 'Toplam Görevler', value: stats.myTasks.length, icon: <Target size={20} />, bgColor: 'bg-blue-100 dark:bg-blue-500/20', textColor: 'text-blue-700 dark:text-blue-300' },
    { title: 'Devam Eden', value: stats.myInProgressTasks.length, icon: <Zap size={20} />, bgColor: 'bg-amber-100 dark:bg-amber-500/20', textColor: 'text-amber-700 dark:text-amber-300' },
    { title: 'Tamamlanan', value: stats.myCompletedTasks.length, icon: <CheckCircle size={20} />, bgColor: 'bg-emerald-100 dark:bg-emerald-500/20', textColor: 'text-emerald-700 dark:text-emerald-300' },
    { title: 'Geciken', value: stats.overdueTasks.length, icon: <AlertCircle size={20} />, bgColor: 'bg-rose-100 dark:bg-rose-500/20', textColor: 'text-rose-700 dark:text-rose-300' }
  ];
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-3 mb-2">
                        <img
                          src={member?.avatar || `https://api.dicebear.com/7.x/avataaars/svg?seed=${member?.fullName || memberId}`}
          ...
          >
            ...
            <div className="relative w-40 h-40 mx-auto mb-6">
              <svg className="transform -rotate-90 w-40 h-40">
                ...
                <motion.circle
                  ...
                  animate={{ strokeDashoffset: 440 - (440 * (stats.myCompletedTasks.length / (stats.myTasks.length || 1))) }}
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                ...
                >
                  {stats.myTasks.length > 0 ? Math.round(stats.myCompletedTasks.length / stats.myTasks.length * 100) : 0}%
                </motion.div>
              </div>
            </div>
            ...
            <div className="space-y-2">
              <div className="flex justify-between p-2 bg-gray-50 dark:bg-gray-700/30 rounded-lg">
                <span className="text-xs text-gray-600 dark:text-gray-400">Biten</span>
                <span className="text-sm font-bold text-gray-900 dark:text-white">{stats.myCompletedTasks.length}</span>
              </div>
              <div className="flex justify-between p-2 bg-gray-50 dark:bg-gray-700/30 rounded-lg">
                <span className="text-xs text-gray-600 dark:text-gray-400">Süreçte</span>
