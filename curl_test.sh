...
echo "🔗 Backend URL: https://flowtasks.preview.emergentagent.com/api"
echo ""

# Step 1: Login
echo "1️⃣ LOGIN TEST"
echo "curl -X POST /api/auth/login"
LOGIN_RESPONSE=$(curl -s -X POST "https://flowtasks.preview.emergentagent.com/api/auth/login" \
...
PROJECTS_RESPONSE=$(curl -s -X GET "https://flowtasks.preview.emergentagent.com/api/projects" \
...
CREATE_RESPONSE=$(curl -s -X POST "https://flowtasks.preview.emergentagent.com/api/tasks" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d "$TASK_DATA")
...
TASKS_RESPONSE=$(curl -s -X GET "https://flowtasks.preview.emergentagent.com/api/tasks?projectId=$PROJECT_ID" \
  -H "Authorization: Bearer $TOKEN")
