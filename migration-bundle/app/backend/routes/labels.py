...
async def get_all_labels(
    ...
    labels = await db.labels.find(query).to_list(1000)
    return labels
...
async def create_label(
    ...
    label_dict["_id"] = str(uuid4()) # Use _id as primary key
    label_dict["createdAt"] = datetime.now(timezone.utc).isoformat()
    
    result = await db.labels.insert_one(label_dict)
    
    created_label = await db.labels.find_one({"_id": label_dict["_id"]})
...
async def update_label(
    ...
    updated_label = await db.labels.find_one({"_id": label_id})
...
async def delete_label(
    ...
    """Delete a label"""
    result = await db.labels.delete_one({"_id": label_id})
